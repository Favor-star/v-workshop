...
renderer.setSize(
  document.getElementsByClassName("screen")[0].offsetWidth,
  document.getElementsByClassName("screen")[0].offsetHeight
);
...
if (WebGL.isWebGLAvailable()) {
  ...
  document.addEventListener("mousedown", (event) =>
    onHandleMouseDown(event, gltfGlobalStorage)
  );
  ...
  // Add GUI controls
  const partControl = gui
    .add(guiParams, "partToRotate", [, "3DGeom-4"])
    .name("Part to Rotate"); //'3DGeom-1', '3DGeom-2', '3DGeom-3', '3DGeom-5', '3DGeom-6'
  const startControl = gui
    .add(guiParams, "startRotation")
    .name("Start Rotation");
